...
function(_mamba_get_url OUT_VAR)
    ...
    elseif(LINUX)
        ...
        elseif(CMAKE_HOST_SYSTEM_PROCESSOR MATCHES "aarch64|arm64")
            ...
            message(
                FATAL_ERROR
                "Unsupported Linux architecture: ${CMAKE_HOST_SYSTEM_PROCESSOR}"
            )
        endif()
...
function(ensure_micromamba OUT_VAR)
    ...
    find_program(
        _micromamba_bin
        micromamba
        PATHS "${CMAKE_SOURCE_DIR}" "${_download_dir}/${_bin_subdir}" NO_CACHE
    )
    ...
    if(_micromamba_bin)
        ...
        message(
            STATUS
            "micromamba not found on system – downloading from ${_mamba_url}"
        )
        ...
        if(_dl_error_code)
            ...
            message(
                FATAL_ERROR
                "Failed to download micromamba: ${_dl_error_msg}"
            )
        endif()
        ...
        execute_process(
            COMMAND
                ${CMAKE_COMMAND} -E tar xf "${_archive}" -- "${_archive_entry}"
        ...
        if(_extract_result)
            message(
                FATAL_ERROR
                "Failed to extract micromamba archive (exit code: ${_extract_result})"
            )
        endif()
        ...
        if(NOT _micromamba_bin)
            message(
                FATAL_ERROR
                "micromamba binary not found after download/extract – check the archive layout"
            )
        endif()
...
function(micromamba_environment)
    ...
    set(_mamba_base_args
        --no-rc
        --root-prefix
        "${CMAKE_BINARY_DIR}/micromamba_root"
    )
    ...
    if(_env_exists)
        message(
            STATUS
            "Updating micromamba environment '${_env_name}' at ${_env_path}"
        )
        execute_process(
            COMMAND
                ${_mamba_bin} update ${_mamba_base_args} -p "${_env_path}"
                ${_channel_args} ${_file_args} ${PARSED_DEPENDENCIES} --yes
                ${_verbose_flag}
        ...
        message(
            STATUS
            "Creating micromamba environment '${_env_name}' at ${_env_path} – this may take a while"
        )
        execute_process(
            COMMAND
                ${_mamba_bin} create ${_mamba_base_args} -p "${_env_path}"
                ${_channel_args} ${_file_args} ${PARSED_DEPENDENCIES} --yes
                ${_verbose_flag}
            WORKING_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}"
            RESULT_VARIABLE _mamba_result
    ...
    if(_mamba_result)
        message(
            FATAL_ERROR
            "micromamba exited with error code ${_mamba_result}"
        )
    endif()
    ...
    if(NOT PARSED_NO_DEPENDS AND PARSED_SPEC_FILE)
        set_property(
            ...
            APPEND
            PROPERTY CMAKE_CONFIGURE_DEPENDS ${PARSED_SPEC_FILE}
        )
    endif()
